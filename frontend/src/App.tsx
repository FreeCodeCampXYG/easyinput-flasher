  ...
  return (
    <div className="page">
      ...
      <div className="about-grid">
        <section className="network-settings"><Network size={20} /><div><strong>网络与代理</strong><p>入口：侧栏“关于与帮助” → 网络与代理。自动模式依次尝试自定义端口、系统代理和直连；不会把 1080 当成唯一入口。</p><div className="network-form"><select value={mode} onChange={(event) => setMode(event.target.value)}><option value="auto">自动探测（端口失败会继续尝试）</option><option value="system">系统/全局代理</option><option value="direct">直连网络</option><option value="custom">自定义代理</option></select><input value={address} onChange={(event) => setAddress(event.target.value)} placeholder="代理端口，如 127.0.0.1:1080" disabled={mode !== "auto" && mode !== "custom"} /><button className="button secondary" type="button" onClick={saveNetwork} disabled={busy}>{busy ? "检测中…" : "保存并测试"}</button><button className="button ghost" type="button" onClick={() => void onCheck()} disabled={busy}>重新测试</button></div><small className={snapshot.network.online ? "ok" : "error"}>{snapshot.network.online ? `GitHub 可访问 · ${snapshot.network.proxyAddress || "已连接"}` : "GitHub 暂不可访问"}</small></div></section>
