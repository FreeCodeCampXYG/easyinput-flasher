...
3. GitHub Actions 在六个原生 runner 构建：Windows 为便携 ZIP，macOS 为 `.app` ZIP，Linux 为 TAR.GZ；每项资产生成 SHA-256 与构建溯源。esptool helper 使用固定 Python 3.13 构建，避免 macOS runner 默认 Python 版本漂移。
