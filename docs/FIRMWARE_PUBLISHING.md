...
如果仓库 Release 只有整片 `factory.bin`，不能把它当作缺失文件的安全替代品。整片镜像的边界可能覆盖配置分区，桌面端不会自动拆分、猜测偏移或扩大写入范围；应在固件仓库的 Actions 中重新产出标准三段镜像、`firmware-manifest.json` 与 `SHA256SUMS.txt`。
